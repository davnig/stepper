...
export type StepPaymentMethodResult = z.infer<typeof FORM_SCHEMA>

export function StepPaymentMethod() {
    const stepperCtx = useStepperContext<StepperResult>()

    const form = useForm<StepPaymentMethodResult>({
    ...
    return (
        <Step
            onNext={async val => {
                ...
                let res: StepPaymentMethodResult
        ...
        >
            <StepHeader />
            <StepContent title='What payment method do you want to use?' className='flex flex-col items-center gap-4'>
                <Form {...form}>
